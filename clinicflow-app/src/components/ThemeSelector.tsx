...
export const ThemeSelector: React.FC = () => {
  const { currentTheme, setTheme, toggleDarkLight, activeOption, isDark } = useTheme();
  ...
  return (
    <div className="flex items-center gap-2" ref={dropdownRef}>
      {/* Botão de Alternância Rápida Claro / Escuro */}
      <button
        onClick={toggleDarkLight}
        className="p-2.5 rounded-xl border border-[var(--border)] hover:border-[var(--accent)] bg-[var(--bg-surface)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] shadow-sm hover:shadow-[0_0_12px_var(--accent-glow)] transition-all duration-200 cursor-pointer flex items-center justify-center"
        title={isDark ? "Alternar para Modo Claro (Light Snow)" : "Alternar para Modo Escuro (Obsidian Cyber)"}
      >
        {isDark ? (
